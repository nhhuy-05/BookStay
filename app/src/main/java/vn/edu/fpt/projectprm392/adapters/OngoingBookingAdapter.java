...
public class OngoingBookingAdapter extends RecyclerView.Adapter<OngoingBookingAdapter.OngoingBookingViewHolder> {
    private List<Place> lists;

    public OngoingBookingAdapter(List<Place> lists) {
        this.lists = lists;
    }
    ...
    public void onBindViewHolder(@NonNull OngoingBookingViewHolder holder, int position) {
        ...
        if (lists.get(position).getStatusRoom().equals("Waiting")) {
            holder.tag_status.setText("Waiting for payment");
            holder.tag_status.setTextColor(holder.tag_status.getResources().getColor(R.color.text_status_wait));
            holder.tag_status.setBackgroundColor(holder.tag_status.getResources().getColor(R.color.bg_status_wait));
        }
        if (lists.get(position).getStatusRoom().equals("Ongoing")) {
            holder.tag_status.setText("OnGoing");
